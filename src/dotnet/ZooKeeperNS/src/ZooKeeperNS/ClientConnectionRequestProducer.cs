    ...
    public class ClientConnectionRequestProducer : IStartable, IDisposable
    {
        ...
        private void StartConnect()
        {
            ...
            do
            {
                ...
                try
                {
                    tempClient.ConnectAsync(zkEndpoints.CurrentEndPoint.ServerAddress.Address, 
        ...
        private void StartConnect()
        {
            ...
            do
            {
                ...
                try
                {
                    Task task = tempClient.ConnectAsync(
                        zkEndpoints.CurrentEndPoint.ServerAddress.Address,
                        zkEndpoints.CurrentEndPoint.ServerAddress.Port);
                    if (!task.Wait(conn.ConnectionTimeout))
                    {
                        Cleanup(tempClient);
                        tempClient = null;
                        throw new TimeoutException();
...
#else
        private void ReceiveAsynchx(byte[] buffer)
        {
            if (Interlocked.CompareExchange(ref isDisposed, 0, 0) == 1)
                return;
            ...
            try
            {
                ...
                {
                    recvCount++;
                    if (buffer == incomingBuffer) // if bData is incoming then surely it is a length information
                    {
                        currentLen = 0;
                        juteBuffer = null;
                        // get the length information from the stream
                        juteBuffer = new byte[ReadLength(buffer)];
                        // try getting other info from the stream
                        ReceiveAsynchx(juteBuffer);
                    }
                    else // not an incoming buffer then it is surely a zookeeper process information
                    {
                        if (Interlocked.CompareExchange(ref initialized, 1, 0) == 0)
